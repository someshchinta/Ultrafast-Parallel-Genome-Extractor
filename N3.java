...
/**
 * Summary description for N3
 *
 */
public class N3 extends JFrame implements ItemListener
{
...
static int start1=0,end1=0;
	...
	public N3(String str_name,int start,int end)
	{
		super();
...
FileWriter fw3 = new FileWriter("timeelapsed-III.txt");
...
send.addActionListener(new ActionListener()
		{
			...
			{
				try{
		Socket cs=new Socket("localhost",3022);
		BufferedReader dis=new BufferedReader(new FileReader("timeelapsed-III.txt"));
...
//
		...
		addComponent(Node1, jScrollPane3, 120,40,234,260);
...
{	if(initial_check_var==0)
	{
	//System.out.println("After while\t"+i_left);
 	i_left = shift_left(text1,pattern1,i_left,n1,(n21-1));
	count1=search_sub(text1,pattern1,i_left,(n21-1),k,count1);
	initial_check_var=1;
	}
	...
	{
		i_left=shift_left(text1,pattern1,(i_left + good_suffix_position),n1,(n21-1));
		count1=search_sub(text1,pattern1,i_left,(n21-1),k,count1);
		match_check_var=0;
	...
	{
		i_left=shift_left(text1,pattern1,(i_left + bad_character_position),n1,(n21-1));
		count1=search_sub(text1,pattern1,i_left,(n21-1),k,count1);
...
//System.out.println("\ti left val\t"+i_left);
...
public static int search_sub(char[] text,char[] patt,int i_left,int j,int[] k,int count11 )
{
 
int j1 = j;
int comp_pos=0;

while ( j1 >=0 && ( text[i_left-k[comp_pos]] == patt[j-k[comp_pos]] ) )
{
j1--;  
comp_pos ++;
}
...
node1_Text.append("\n Found at position\t"+(i_left-j));
good_suffix_position = ( (i_left+j < n1)? j-badchar10[text[i_left+j]] : 1 );
//System.out.println("\tgood suffix posituion\t"+good_suffix_position);
...
{
if(i_left<n1-n21)bad_character_position= max(1, j - badchar10[text[i_left+j]]);
//System.out.println("\tbad_character_position\t"+bad_character_position);
}
